@extends(Auth::user()->role === 'lecturer' ? 'layouts.lecturer_dashboard' : 'layouts.student_dashboard')
                ...
                <div class="row align-items-center g-4">
                    <div class="col-md-auto text-center">
                        <div class="position-relative d-inline-block">
                            ...
                            <div id="avatarPreview" class="profile-avatar bg-primary text-white d-flex align-items-center justify-content-center fw-bold fs-1 overflow-hidden" 
                                style="width: 128px; height: 128px; border-radius: 50%; border: 4px solid #fff; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); background-image: url('{{ $user->avatar ? asset('storage/' . $user->avatar) : '' }}'); background-size: cover; background-position: center;">
                                @if(!$user->avatar)
                    ...
                    <div class="col-md">
                        <div class="d-flex align-items-center gap-2 mb-1 justify-content-center justify-content-md-start">
                            <h2 class="h4 fw-bold mb-0">{{ $user->name }}</h2>
                            <span class="badge rounded-pill bg-primary-subtle text-primary border border-primary-subtle px-3">{{ ucfirst($user->role) }}</span>
                        </div>
                        <p class="text-muted text-center text-md-start">{{ $user->email }}</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-bold small text-secondary">
                                @if ($user->role == 'student') Student ID 
                                @else Teacher ID 
                                @endif
                            </label>
                            <input type="text" name="code" class="form-control" value="{{ old('code', $user->code) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-bold small text-secondary">Role</label>
                            <input type="text" class="form-control bg-light" disabled value="{{ ucfirst($user->role) }}">
                        </div>
...
<div class="modal fade" id="deleteAccountModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form method="post" action="{{ route('profile.destroy') }}" class="modal-content border-0 shadow-lg" style="border-radius: 1rem;">
            @csrf
            @method('delete')
