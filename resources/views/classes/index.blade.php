...
<div class="container-xxl">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-4 mb-5">
        ...
        @if(Auth::user()->role === 'lecturer')
            <a href="{{ route('classes.create') }}" class="btn btn-primary-custom transition-all">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-2 mb-5">
        <form action="{{ route('classes.index') }}" method="GET" class="row g-2 align-items-center w-100 m-0">
            <div class="col-md-8 position-relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="form-control border-0 ps-5 shadow-none" placeholder="Search by class name or lecturer...">
            </div>
            <div class="col-md-4 d-flex align-items-center border-start ps-3">
                ...
                <select name="sort" class="form-select form-select-sm border-0 bg-transparent shadow-none fw-medium" onchange="this.form.submit()">
                    ...
                    <option value="academic_year" {{ request('sort') == 'academic_year' ? 'selected' : '' }}>Academic Year</option>
                </select>
            </div>
        </form>
    </div>
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="class-card p-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-4 pb-4 border-bottom">
                    <img src="{{ $class->lecturer->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode($class->lecturer->name ?? 'Unknown').'&background=f1f5f9&color=475569' }}" class="lecturer-avatar" alt="{{ $class->lecturer->name ?? 'Lecturer' }}">
                    <div>
                        <div class="text-uppercase text-muted fw-bold" style="font-size: 0.6rem; letter-spacing: 0.05em">Lecturer</div>
                        <div class="fw-bold small">{{ $class->lecturer->name ?? 'Unknown' }}</div>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-6">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <span class="fw-bold small text-nowrap">{{ $class->semester }}</span>
                        </div>
                    </div>
                    <div class="col-6">
                    ...
                    <div class="col-6 d-flex align-items-center gap-2">
                        ...
                        <span class="small fw-bold">{{ $class->students_count ?? $class->students->count() }} Students</span>
                    </div>
                    <div class="col-6 d-flex align-items-center gap-2">
                        <span class="material-symbols-outlined text-muted" style="font-size: 18px">group</span>
                        <span class="small fw-bold">{{ $class->groups->count() }} Groups</span>
                    </div>
                </div>

                <a href="{{ route('classes.show', $class) }}" class="btn btn-outline-light text-dark border-secondary-subtle w-100 fw-bold py-2 mt-auto d-flex align-items-center justify-content-center gap-2">
    ...
    <div class="text-center mt-5">
        <p id="resultsCount" class="text-muted fw-medium small">Showing {{ $classes->count() }} active classes</p>
    </div>
</div>
@endsection
